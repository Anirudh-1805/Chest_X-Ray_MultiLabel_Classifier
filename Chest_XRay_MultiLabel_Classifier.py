...
def check_for_leakage(df1, df2, patient_col):
    ...
    patients_in_both_groups = list(set(df1_patients_unique).intersection(df2_patients_unique))
...
def compute_class_freqs(labels):
    ...
    positive_frequencies = np.sum(N == 1,axis=0) / N.shape[0]
    negative_frequencies = np.sum(N == 0,axis=0) / N.shape[0]
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        # For each class, calculate the weighted loss
        for i in range(len(pos_weights)):
            

            # Add both losses for this class
